...
{
	return new Unit_FireMog();
}

Unit_FireMog::Unit_FireMog(void)
{
}
...
void Unit_FireMog::Shot(float x, float y, bool isEnemy, bool blockHit)
{
	if( !m_AttackEnable )
	{
		return;
	...
	damageData.x = 300.0f;
	damageData.y = 15.0f;
	TaskSet_Shot( m_Power, UnitBase::DAMAGETYPE_FIRE, damageData, m_x, m_y, x, y, isEnemy, blockHit, this);
}

void Unit_FireMog::MovePoint(void)
{
...
void Unit_FireMog::FreeMove(void)
{
	VECTOR position;
	position.x = m_x;
	position.y = m_y;
...
void Unit_FireMog::EnemyMove(void)
{
	// 移動
	...
	else
	{
		...
		{
			if( block == NULL || (block && block->m_ID != BLOCK_FIRE) )
			{
				m_x += facing.x * m_Speed;
				m_y += facing.y * m_Speed;
			}
